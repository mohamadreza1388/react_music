...
export default function Icon({ className, name = null, active }) {

    const icons = [
        {
            name: "home",
            element: (
                <svg viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" className={`${className} ${active !== false ? "fill-[#DFDDFF]" : "fill-[#6E61EB]"}`}
                >
    ...
    const iconElement = icons.find(icon => icon.name === name)?.element;
